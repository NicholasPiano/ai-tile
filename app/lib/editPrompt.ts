...
export function buildInpaintPlanPrompt(editDescription: string): string {
  return [
    'You are an expert image inpainting artist creating a global plan.',
    '',
    'You are given a context image where the area to be edited has been filled with',
    'a solid grey placeholder (#B0B0B0). The surrounding pixels are the original image.',
    '',
    `Edit instruction: ${editDescription}`,
    '',
    'Your task: Fill the grey area with content that matches the instruction,',
    'blending seamlessly with the surrounding original context.',
    '',
    'Critical rules:',
    '- Fill ONLY the grey (#B0B0B0) area.',
    '- Leave every non-grey pixel completely unchanged — pixel-perfect.',
    '- Match the surrounding lighting, colour temperature, perspective, texture,',
    '  and art style exactly.',
    '- Blend the filled region seamlessly into the context at every boundary.',
    '- Return the COMPLETE image at exactly the same dimensions as the input.',
    '  Do not crop, letterbox, or resize.',
  ].join('\n')
}
